...
const modalEnabled = computed(() => props.open)
const createAssignableUsersQuery = useCreateAssignableUsers(activeIssueType, effectiveParentKey, modalEnabled)
const createPrioritiesQuery = useCreatePriorities(activeIssueType, effectiveParentKey, modalEnabled)
...
const issueTypeOptions = computed<JiraCreateIssueType[]>(() => (
  getAllowedIssueTypesForParent(selectedParentTicket.value?.issueType ?? null)
))
...
const supportedParentTickets = computed(() => {
  ...
  return [...props.tickets]
    ...
    .sort((left, right) => `${left.key} ${left.summary}`.localeCompare(`${right.key} ${right.summary}`, undefined, { sensitivity: 'base' }))
})
...
function resetForm() {
  ...
  selectedIssueType.value = getDefaultIssueType(initialParentTicket?.issueType ?? null, props.initialIssueType)
...
function startEditingParent() {
  if (createMutation.isPending.value || props.parentLocked || !supportedParentType.value) return
...
async function submit() {
  if (!selectedIssueType.value || createMutation.isPending.value) return
      ...
      >
        <div class="w-full max-w-2xl rounded-3xl border border-white/[0.08] bg-surface-1 shadow-2xl shadow-black/50">
          <div class="flex items-start justify-between border-b border-white/[0.06] px-6 py-5">
            <div>
              <p class="text-[11px] uppercase tracking-[0.18em] text-slate-500">Create Issue</p>
              <h2 class="mt-1 text-lg font-semibold text-white">New {{ selectedIssueType || 'Issue' }}</h2>
            </div>
            <button
              type="button"
            ...
            <div class="space-y-2">
              ...
              <div class="flex flex-wrap gap-2">
                <button
                  ...
                  :disabled="issueTypeLocked || createMutation.isPending.value"
              ...
              <div
                ...
                :class="[
                  ...
                  parentLocked
                    ...
                    : '',
                  createMutation.isPending.value ? 'opacity-70' : '',
                ]"
              >
                ...
                <button
                  ...
                  :disabled="parentLocked || createMutation.isPending.value"
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <select
                  ...
                  :value="getTextValue(field.key)"
                  class="w-full rounded-2xl border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-sm text-slate-200 outline-none transition focus:border-indigo-500/40"
                  :disabled="createMutation.isPending.value || isCreateFieldLoading(field.key)"
              ...
              <div class="flex flex-wrap items-start gap-3">
                  ...
                  <div class="group relative flex items-center gap-1.5">
                    <div class="flex items-center gap-2">
                      <select
                        ...
                        :disabled="createMutation.isPending.value || isCreateFieldLoading('priority')"
                ...
                <div class="min-w-0 flex-1 space-y-2">
                  <label for="create-field-assignee" class="flex items-center gap-2 text-sm font-medium text-slate-200">
                    <span>Assignee</span>
                  </label>
          ...
          <div class="flex items-center justify-end gap-3 border-t border-white/[0.06] px-6 py-4">
            ...
            <button
              ...
              :disabled="createMutation.isPending.value || !selectedIssueType"
              @click="submit"
            >
              {{ createMutation.isPending.value ? 'Creating...' : `Create ${selectedIssueType || 'Issue'}` }}
            </button>
          </div>
        </div>
      </div>
