...
) => ['create-assignees', issueType, parentKey] as const
...
export function useCreateAssignableUsers(
  ...
  return useQuery({
    queryKey: computed(() => createAssignableUsersQueryKey(issueType.value, parentKey.value)),
    queryFn: () => {
      ...
      return fetchCreateAssignableUsers(nextIssueType, parentKey.value)
    },
